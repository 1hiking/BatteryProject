...
separator_ver = ttk.Separator(mainframe, orient="vertical").grid(column=2, row=1,
                                                                 sticky=(N, S), rowspan=50)  # Not necessary to be 50


# Second column
if battery_data.power_plugged:
    ttk.Label(mainframe, text="Yes").grid(
        column=3, row=3, sticky=(W, E), pady=5)
else:
    ttk.Label(mainframe, text="Nah").grid(
        column=3, row=3, sticky=(W, E), pady=5)

# why do you do this to me
x = '{}%'.format(battery_data.percent)
ttk.Label(mainframe, text=x).grid(
    column=3, row=1, sticky=(W, E), pady=5)
